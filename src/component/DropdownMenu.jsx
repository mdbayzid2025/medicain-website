...
const DropdownMenu = ({ openMenu, setOpenMenu, }) => {
  ...
  return (
    <div
      ...
      <button
        onClick={signUp}
        className="text-center text-lg block mb-3 w-full bg-primaryColor text-white py-1.5 rounded-md font-semibold" >Sign Out</button>
    </div>
  )
}
...
className={`${!openMenu ? "-top-full  " : "top-28  opacity-1 "} absolute bg-[var(--my-color)] rounded-lg shadow-md px-2 w-full transition-all duration-500 `}>
    ...
    <button
    onClick={()=>signUp()}
    className="text-center text-lg block my-3 w-full bg-primaryColor py-1.5 rounded-md" >Sign Out</button>
</div>
*/