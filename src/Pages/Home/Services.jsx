                ...
                {serviceData && serviceData.map((data, index) =>
                    <div key={index} className='relative bg-[var(--menu-bg)] p-4 pb-16'>
                        ...
                        <div className="absolute w-full bottom-0 flex justify-between items-center mt-8">
                            <Link to={`/doctors?category=${data.name}`} className=" flex items-center justify-center w-12 h-12 border-2 rounded-full hover:bg-primaryColor hover:border-none text-xl hover:text-white">
                                < BsArrowRight />
                            </Link>
