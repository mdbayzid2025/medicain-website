            ...
            <div className="container text-center">
              ...
              <div className="w-full md:max-w-[500px]  mx-auto bg-[#0066ff2c] rounded-md flex items-center justify-between">
                <input type="search" onChange={(e) => setSearchInput(e.target.value)}
                  ...
                  name="" />
                <button
                  className="py-3 px-8 bg-primaryColor text-white mt-0 rounded-none rounded-r-md">
                  Search
                </button>
              ...
              {doctors && doctors.map((doctor, index) =>
                <div key={index} className="mx-auto rounded-lg bg-slate-100 w-full h-full flex flex-col  shadow-sm">
                  ...
                  <div className='rounded-b-lg p-4 flex flex-col justify-between flex-1'>
                    ...
                    <Link to={`/doctors/${doctor._id}`} > <div className="flex items-center justify-between mt-4 bg-primaryColor p-2 rounded-md !text-white">
                      <p className="text-textColor">{doctor?.experience[0]?.hospital ?? 'Central Hospital'}</p>
                      <span className="flex items-center justify-center text-lg border-2 border-slate-400 hover:border-none hover:bg-primaryColor hover:text-white w-10 h-10 rounded-full">
                        <BsArrowRight />
                      </span>
                    </div>
                    </Link>
