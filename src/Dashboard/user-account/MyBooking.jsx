        ...
        {user && user?.appointment?.map((data, index) =>
          <div key={index} className="mx-auto w-full">
            ...
            <Link
              ...
              className="group flex bg-primaryColor text-white items-center justify-between mt-5 p-1 rounded-r-full">

              <p className="text-textColor pl-1">About Doctor</p>

              <p className="flex items-center justify-center text-lg border-2 group-hover:border-none group-hover:bg-primaryColor hover:text-white w-10 h-10 rounded-full">
                <BsArrowRight />
              </p>
            </Link>
          </div>)}
